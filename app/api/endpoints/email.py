...
from app.api.dependencies import (
    get_current_user,
    UnitOfWorkDep,
    email_service as email_service_dep,
    user_service as user_service_dep,
)
...
async def verify_email(token: str, unit_of_work: UnitOfWorkDep, service: email_service_dep):
    return await service.verify_token(token=token, unit_of_work=unit_of_work)


@router.get("/verify-email-change")
async def verify_email_change(token: str, user_service: user_service_dep, uow: UnitOfWorkDep):
...
async def request_email_change(
    ...
    token = await user_service.initiate_email_change(
        user_id=current_user.id,
        new_email=change_email_request.new_email,
        password=change_email_request.password,
